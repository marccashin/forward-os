...
async function getInstallationToken(appId, privateKey) {
  ...
  const headers = { 'Authorization': 'Bearer ' + jwt, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json' };
  const installations = await fetch('https://api.github.com/app/installations', { headers }).then(r => r.json());
  if (!Array.isArray(installations) || installations.length === 0) throw new Error('No installations: ' + JSON.stringify(installations));
  const installationId = installations[0].id;
  const tokenRes = await fetch('https://api.github.com/app/installations/' + installationId + '/access_tokens', { method: 'POST', headers }).then(r => r.json());
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await fetch('https://api.github.com/repos/' + repo + '/contents/' + filename, { method: 'PUT', headers: h, body: JSON.stringify(putBody) }).then(r => r.json());
